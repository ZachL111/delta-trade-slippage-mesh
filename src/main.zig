...
test "fixture decisions" {
    const signal_case_1 = Signal{ .demand = 74, .capacity = 83, .latency = 19, .risk = 11, .weight = 8 };
    try std.testing.expectEqual(@as(i32, 140), score(signal_case_1));
    try std.testing.expect(std.mem.eql(u8, classify(signal_case_1), "review"));
    const signal_case_2 = Signal{ .demand = 68, .capacity = 79, .latency = 12, .risk = 17, .weight = 11 };
    try std.testing.expectEqual(@as(i32, 121), score(signal_case_2));
    try std.testing.expect(std.mem.eql(u8, classify(signal_case_2), "review"));
    const signal_case_3 = Signal{ .demand = 89, .capacity = 71, .latency = 19, .risk = 17, .weight = 9 };
...
test "domain review lane" {
    const item = DomainReview{ .signal = 45, .slack = 29, .drag = 30, .confidence = 94 };
    try std.testing.expectEqual(@as(i32, 123), domainReviewScore(item));
